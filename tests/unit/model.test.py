...
def train_tfidf_svm(X, y):
    ...
    vectorizer = TfidfVectorizer(
        ...
        max_features=1000,
        min_df=1,
        sublinear_tf=True,  # Reduced for testing
    )
    ...
    if min_samples_per_class >= 2:
        ...
        )  # Reduced max_iter for testing
        grid_search = GridSearchCV(
            svm, param_grid, cv=safe_cv_folds, scoring="f1_weighted"
        )  # Reduced cv for testing
        grid_search.fit(X_tfidf, y)

        # Train with calibrated classifier using best model
        best_svm = grid_search.best_estimator_
        calibrated_svm = CalibratedClassifierCV(
            best_svm, cv=safe_cv_folds
        )  # Reduced cv for testing
...
class TestBasicFunctionality:
    ...
    def test_load_data_missing_columns(self):
        ...
        try:
            ...
            with pytest.raises(
                ValueError, match="must contain 'Document' and 'Topic_group' columns"
            ):
                load_data(temp_file)
